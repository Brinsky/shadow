...
public class LLVMOutput extends AbstractOutput {
    ...
    for (Type type : moduleType.getUsedTypes()) {
      if (!type.isParameterized()
        ...
        if (!moduleType.encloses(type)) {
          ...
          writer.write('@' + raw(type, "_instance") + " = external global " + type(type));
        }
      }
      ...
      interfaceData
          ...
          .append(
              "]} { %ulong -1, "
                  + type(Type.GENERIC_CLASS)
                  + " "
                  + classOf(new ArrayType(Type.METHOD_TABLE))
                  + ", "
                  + methodTableType(Type.ARRAY)
                  + " "
      ...
      interfaceClasses
          ...
          .append(
              "]} { %ulong -1, "
                  ...
                  + methodTable(Type.ARRAY)
                  + ", "
                  + typeLiteral((long) interfaceCount)
                  + ", ["
                  + interfaceCount
                  + " x "
                  + type(Type.CLASS)
  ...
  private void writeExceptions() throws ShadowException {
    ...
    // MethodSignature isSubtype = Type.CLASS.getMatchingMethod("isSubtype", new
    // SequenceType(Type.CLASS));
    ...
    // i1 @shadow.standard..Class_MisSubtype_shadow.standard..Class(%shadow.standard..Class*,
    // %shadow.standard..Class*)
    // @"??_7type_info@@6B@" = external global i8*

    for (ExceptionType exceptionType : exceptions) {
      writer.write(
          "define linkonce_odr i32 " + exceptionMethod(exceptionType) + "(i8* %0, i8* %1) {");
      writer.indent();
      writer.write(
      ...
      writer.outdent();
      writer.write("}");
      // writer.write(exceptionTable(exceptionType) + " = private unnamed_addr constant { [2 x i8*]
      // } { [2 x i8*] [i8* bitcast (i64 (i8*)* @__exceptionHashCode to i8*), i8* bitcast (i32 (i8*,
      // i8*)* " + exceptionMethod(exceptionType) + " to i8*)] }");
      // writer.write(exceptionData(exceptionType) + " = linkonce_odr global { i8**, i8*, [8 x i8] }
      // { i8** bitcast ({ [2 x i8*] }* " + exceptionTable(exceptionType) + " to i8**), i8* null, [8
      // x i8] c\"Shadow\\00\\00\" }");
      // writer.write(exceptionData(exceptionType) + " = linkonce_odr global { i8**, i8*, [8 x i8] }
      // { i8** bitcast ({ [2 x i8*] }* " + exceptionTable(exceptionType) + " to i8**), i8* null, [8
      // x i8] c\"Shadow\\00\\00\" }");
    ...
    if (node.getOuterType() instanceof InterfaceType) {
      ...
      writer.write(
          nextTemp()
              ...
              + ", i32 0, i32 "
              + node
                  .getIndex()); // may need to + 1 to the node.getIndex() if a parent method table
                                // is added
  ...
  public void visit(TACChangeReferenceCount node) throws ShadowException {
    ...
    if (node.isIncrement()) {
      ...
      if (node.isField()) {
        ...
        writer.write(nextTemp() + " = load " + type(field) + ", " + typeText(field, temp(1), true));
        name = temp(0);
      } else if (variable.getOriginalName().equals("this")) name = "%0";
      else {
        writer.write(
            nextTemp()
                + " = load "
  ...
  private void visitShiftOperation(TACBinary node, String instruction) throws ShadowException {
  ...
  public void visit(TACCleanupPad node) throws ShadowException {
    ...
    if (parentBlock != null) parentPad = parentBlock.getCleanupPad();

    String parent;
    if (parentPad == null) parent = "none";
    else parent = symbol(parentPad);
  ...
  public void visit(TACCatchPad node) throws ShadowException {
    ...
    writer.write(
        padToken
            ...
            + " to i8*)]");
    // writer.write(nextTemp(node) + " = catchpad within " + symbol(node.getCatchSwitch()) + " [i8*
    // bitcast (i32 (i8*, i8*)* " + exceptionData(node.getType()) + " to i8*)]");

    // writer.write(nextTemp(node) + " = catchpad within " + symbol(node.getCatchSwitch()) + " [{
    // i8**, i8*, [8 x i8] }* " + exceptionData(node.getType()) + ", i32 8, " +
    // type(node.getVariable()) + "* " + name(node.getVariable()) + "]");
    // writer.write(nextTemp(node) + " = catchpad within " + symbol(node.getCatchSwitch()) + " [i8*
    // bitcast (i32 (i8*, i8*)* " + exceptionMethod(node.getType()) + " to i8*), i32 8, " +
    // type(node.getVariable()) + "* " + name(node.getVariable()) + "]");
    ...
    writer.write(
        nextTemp(node)
            ...
            + " to "
            + type(node.getType()));
    // writer.write("store " + type(node.getVariable()) + " " + temp(0) + ", " +
    // type(node.getVariable()) + "* " +  name(node.getVariable()));
  ...
  private static String methodToString(
    ...
    if (parameters) {
      ...
      for (int i = 0; i < parameterTypes.size(); i++) {
        ...
        } else sb.append(", ").append(type(parameterTypes.get(i)));
      }
      sb.append(')');
    }
    return sb.toString();
  ...
  private static String methodToString(TACMethodName method, boolean name, boolean parameters) {
  ...
  protected static String type(Type type, boolean nullable) {
    ...
    if (type instanceof MethodType) return type((MethodType) type, nullable);
  ...
  */
  ...
  private static String name(TACConstant constant) {
    return new StringBuilder("@")
        .append(raw(constant.getFieldKey().parentType, "_C" + constant.getFieldKey().fieldName))
        .toString();
  }
    ...
    for (ModifiedType parameter : parameterList) {
      ...
      if (parameterType instanceof InterfaceType)
        ...
        tables
            ...
            .append(
                " bitcast ("
                    + methodTableType(parameterWithoutArguments)
                    + " "
                    + methodTable(parameterWithoutArguments)
                    + " to "
                    + type(Type.METHOD_TABLE)
                    + ")");
    }
