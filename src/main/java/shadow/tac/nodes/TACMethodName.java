...
public class TACMethodName extends TACOperand implements TACMethodRef {
  ...
  public TACMethodName(TACNode node, TACOperand prefixNode, MethodSignature sig) {
    ...
    if (sig.isWrapper()) wrapped = new TACMethodName((TACNode) this, sig.getWrapped());
  }
  ...
  private TACMethodName(
      TACNode node, TACOperand prefixNode, MethodSignature sig, TACMethodName otherWrapped) {
    super(node);
    initialize(prefixNode, sig);
    if (otherWrapped != null) wrapped = new TACMethodName((TACNode) this, otherWrapped);
  }
