...
/**
 * TAC representation of class constant Example: Object:class
 *
 * @author Jacob Young
...
public class TACClass extends TACOperand {
  ...
  public class TACClassData extends TACOperand {
    ...
    @Override
    public Type getType() {
      if (type instanceof ArrayType || type.isParameterized()) return Type.GENERIC_CLASS;
  ...
  public class TACMethodTable extends TACOperand {
    ...
    @Override
    public Type getType() {
      if (type instanceof InterfaceType) return Type.METHOD_TABLE;
      else if (type instanceof ArrayType)
        return new MethodTableType(((ArrayType) type).convertToGeneric());
      else return new MethodTableType(type);
    }
  ...
  public TACClass(TACNode node, Type classType) {
    ...
    if (type instanceof TypeParameter) {
      ...
      for (ModifiedType parameter : outer.getTypeParameters()) {
        if (parameter.getType().getTypeName().equals(classType.getTypeName())) break;
        index++;
      }

      if (index == outer.getTypeParameters().size())
        throw new IllegalArgumentException(
            "Type parameter " + classType.getTypeName() + " not present in class " + outer);
      ...
      TACLoad classValue =
          new TACLoad(
              this,
              new TACFieldRef(
                  new TACLocalLoad(this, _this),
                  new SimpleModifiedType(Type.CLASS, new Modifiers(Modifiers.IMMUTABLE)),
                  "class"));
      TACOperand genericClass =
          TACCast.cast(this, new SimpleModifiedType(Type.GENERIC_CLASS), classValue);
      ...
      classData =
          new TACLoad(
              this,
              new TACArrayRef(
                  this, generics, new TACLiteral(this, new ShadowInteger((long) index)), false));
      ...
      methodTable =
          new TACLoad(
              ...
              new TACArrayRef(
                  this,
                  methodTables,
                  new TACLiteral(this, new ShadowInteger((long) index)),
                  false));
    } else {
  ...
  private TACOperand lookUpClass(Type type, Type outer) {
    if (outer instanceof ClassType) {
      ...
      if (i < size) {
        ...
        TACOperand classData =
            new TACLoad(
                ...
                new TACArrayRef(
                    ...
                    new TACLiteral(
                        this,
                        new ShadowInteger((long) (i + outerClass.getTypeParameters().size()))),
                    false));
        ...
        throw new IllegalArgumentException(
            "Type " + type + " not found in dependency list for class " + outer);

    } else throw new IllegalArgumentException("Cannot look up type parameter in non-class type");
  }
  ...
  public int getNumOperands() {
    ...
    else if (methodTable == null) return 1;
    return 2;
  }
  ...
  public TACOperand getOperand(int num) {
    if (num < getNumOperands()) {
      if (num == 0) return classData;
      else if (num == 1) return methodTable;
    }
    throw new IndexOutOfBoundsException("" + num);
  }
