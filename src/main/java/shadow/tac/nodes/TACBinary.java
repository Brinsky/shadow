...
/**
 * TAC representation of binary operator Example: x + y
 *
 * @author Jacob Young
...
public class TACBinary extends TACUpdate {
  ...
  public TACBinary(TACNode node, TACOperand firstOperand, Boolean op, TACOperand secondOperand) {
    this(
        node,
        firstOperand,
        new SimpleModifiedType(Type.BOOLEAN),
  ...
  public TACBinary(
    ...
    this(
        ...
        op,
        secondOperand,
        signature.getParameterTypes().get(0),
        isCompare ? new SimpleModifiedType(Type.BOOLEAN) : signature.getReturnTypes().get(0));
  }

  private TACBinary(
      TACNode node,
      TACOperand firstOperand,
      ModifiedType firstType,
      String op,
      TACOperand secondOperand,
      ModifiedType secondType,
    ...
    else second = check(secondOperand, secondType);
    result = resultType;
  }
  ...
  public boolean update(Set<TACUpdate> currentlyUpdating) {
    ...
    if ((changed || getUpdatedValue() == null)
        && firstValue instanceof TACLiteral
        && secondValue instanceof TACLiteral) {
      try {
