...
public class TypeCollector extends ScopedChecker {
  ...
  /**
   * Calls <code>collectTypes</code> with multiple, non-main files. This proxy method is usually
    ...
    while (!uncheckedFiles.isEmpty()) {
      ...
      if (Files.exists(canonicalFile)) {
        ...
        if (!useSourceFiles
            ...
            (typeCheckOnly
                || (Files.exists(llvm)
                    && Files.getLastModifiedTime(llvm).compareTo(Files.getLastModifiedTime(meta))
                        >= 0))) canonicalFile = meta;
    ...
    if (node != null) {
      ...
      if (unqualifiedName != null) {
        ...
        if (!text.equals("default")) {
          ...
          for (int i = 0; i < parts.length; ++i) parent = parent.getParent();
        }
      }
    }
  ...
  private Type createType(
    ...
    if (lookupType(node, typeName) != null) {
      ...
      if (currentType != null) currentType.addInnerType(name, (ClassType) type);

      // Special case for standard types needed in the compiler.
      if (currentPackage.getQualifiedName().equals("shadow:standard")) {
        switch (typeName) {
          case "AddressMap":
            Type.ADDRESS_MAP = (ClassType) type;
            break;
          case "Array":
            Type.ARRAY = (ClassType) type;
            break;
          case "ArrayNullable":
            Type.ARRAY_NULLABLE = (ClassType) type;
            break;
          case "AssertException":
            Type.ASSERT_EXCEPTION = (ExceptionType) type;
            break;
          case "CanAdd":
            Type.CAN_ADD = (InterfaceType) type;
            break;
          case "CanCompare":
            Type.CAN_COMPARE = (InterfaceType) type;
            break;
          case "CanDivide":
            Type.CAN_DIVIDE = (InterfaceType) type;
            break;
          case "CanEqual":
            Type.CAN_EQUAL = (InterfaceType) type;
            break;
          case "CanModulus":
            Type.CAN_MODULUS = (InterfaceType) type;
            break;
          case "CanMultiply":
            Type.CAN_MULTIPLY = (InterfaceType) type;
            break;
          case "CanNegate":
            Type.CAN_NEGATE = (InterfaceType) type;
            break;
          case "CanSubtract":
            Type.CAN_SUBTRACT = (InterfaceType) type;
            break;
          case "CanIndex":
            Type.CAN_INDEX = (InterfaceType) type;
            break;
          case "CanIndexNullable":
            Type.CAN_INDEX_NULLABLE = (InterfaceType) type;
            break;
          case "CanIndexStore":
            Type.CAN_INDEX_STORE = (InterfaceType) type;
            break;
          case "CanIndexStoreNullable":
            Type.CAN_INDEX_STORE_NULLABLE = (InterfaceType) type;
            break;
          case "CanIterate":
            Type.CAN_ITERATE = (InterfaceType) type;
            break;
          case "CanIterateNullable":
            Type.CAN_ITERATE_NULLABLE = (InterfaceType) type;
            break;
          case "CastException":
            Type.CAST_EXCEPTION = (ExceptionType) type;
            break;
          case "Class":
          ...
          case "float":
            ...
            break;
          case "GenericClass":
            Type.GENERIC_CLASS = (ClassType) type;
            break;
          case "IndexOutOfBoundsException":
            Type.INDEX_OUT_OF_BOUNDS_EXCEPTION = (ExceptionType) type;
            break;
          case "int":
            Type.INT = (ClassType) type;
            break;
          case "Integer":
            Type.INTEGER = (InterfaceType) type;
            break;
          case "InterfaceCreateException":
            Type.INTERFACE_CREATE_EXCEPTION = (ExceptionType) type;
            break;
          case "Iterator":
            Type.ITERATOR = (InterfaceType) type;
            break;
          case "IteratorNullable":
            Type.ITERATOR_NULLABLE = (InterfaceType) type;
            break;
          case "long":
          ...
          case "Method":
            ...
            break;
          case "MethodTable":
            Type.METHOD_TABLE = (ClassType) type;
            break;
          case "Number":
          ...
          case "UnexpectedNullException":
            Type.UNEXPECTED_NULL_EXCEPTION = (ExceptionType) type;
            break;
          case "ushort":
            Type.USHORT = (ClassType) type;
            break;
          case "CanRun":
            Type.CAN_RUN = (InterfaceType) type;
            break;
          case "Thread":
            Type.THREAD = (ClassType) type;
            break;
        }
      }
  ...
  private Path findPath(String name) {
    ...
    for (Path importPath : importPaths) {
      ...
      if (isDirectory) {
        ...
        if (Files.exists(shadowVersion)) return shadowVersion;
        else if (Files.exists(metaVersion)) return metaVersion;
      }
    }
    ...
    // There's an identifier that could be a class, but it's got to have a suffix.
      ...
      if (findSymbol(symbol) == null
        ...
        if (suffix.classSpecifier() != null
          ...
          if (importedTypes.containsKey(symbol)) usedTypes.add(importedTypes.get(symbol).path);
          else
            addError(
