...
public abstract class BaseChecker extends ShadowVisitorErrorReporter {
   ...
   */
  protected static final List<ShadowException> isValidInitialization(
   ...
   * @return list of errors (empty if valid)
   */
  protected static final List<ShadowException> isValidAssignment(
      ...
      if (outer instanceof ClassType) {
        ...
        while (parent != null) {
          if (node.getEnclosingType().equals(parent)) {
            if (node.getModifiers().isPrivate()) return false;
            else return true;
          }
          parent = parent.getExtendType();
        }
