...
public class MethodType extends Type {
  ...
  protected SequenceType parameterTypes;
  /** List of parameter types */
  protected SequenceType returns;
  /** List of return types */
  protected boolean inline = false;
  /** Whether the method is defined inline */
  public MethodType() {
  ...
  public ModifiedType getParameterType(String paramName) {
    for (int i = 0; i < parameterNames.size(); ++i) {
      if (parameterNames.get(i).equals(paramName)) return parameterTypes.get(i);
    }
  ...
  public boolean containsParam(String paramName) {
    for (int i = 0; i < parameterNames.size(); ++i) {
      if (parameterNames.get(i).equals(paramName)) return true;
    }
  ...
  public String toString(int options) {
    if ((options & MANGLE) != 0) return parameterTypes.toString("", "", options);
  ...
  @Override
  public MethodType replace(List<ModifiedType> values, List<ModifiedType> replacements)
  ...
  public boolean matchesInterface(MethodType type) {
    boolean value =
        returns.isSubtype(type.returns) && type.parameterTypes.isSubtype(parameterTypes);
    return value;
  }
