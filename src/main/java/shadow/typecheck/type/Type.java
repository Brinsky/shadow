...
public abstract class Type implements Comparable<Type> {
  ...
  private LinkedHashMap<String, ShadowParser.VariableDeclaratorContext> fieldTable =
      new LinkedHashMap<String, ShadowParser.VariableDeclaratorContext>();
  ...
  private HashMap<String, List<MethodSignature>> methodTable =
      new HashMap<String, List<MethodSignature>>();
  ...
  public void addInstantiation(
    ...
    for (ModifiedType argument : arguments) {
      if (types.children == null) types.children = new ArrayList<TypeArgumentCache>();
  ...
  public boolean equals(Type type) {
    if (type != null) {
      ...
      if (getPackage() == type.getPackage() && type.getTypeName().equals(getTypeName())) {
        if (isParameterized() && !type.typeParameters.matches(typeParameters)) return false;
  ...
  protected boolean isNumericalSubtype(Type t) {
    ...
    } else if (this.equals(CODE)) // just like uint?
    {
      return t.equals(UINT)
          ...
          || t.equals(FLOAT)
          || t.equals(DOUBLE);
      // return t.equals(INT) || t.equals(UINT) || t.equals(LONG) || t.equals(ULONG) ||
  ...
  public final boolean recursivelyContainsField(String fieldName) {
  ...
  public static ClassType makeSigned(ClassType type) {
    if (type.equals(UBYTE)) return BYTE;

    if (type.equals(USHORT)) return SHORT;

    if (type.equals(UINT)) return INT;

    if (type.equals(ULONG)) return LONG;
  ...
  // for cases where integers are required (bitwise operations, array bounds, switch statements,
  // etc.)
  ...
  public List<MethodSignature> getMethodOverloads(String methodName) {
    ...
    if (signatures == null) return new ArrayList<MethodSignature>();
  ...
  public int getMethodIndex(MethodSignature method) {
    ...
    if (methodIndexCache == null) {
      Map<MethodSignature, Integer> cache = new HashMap<MethodSignature, Integer>();
  ...
  protected List<MethodSignature> orderMethods(List<MethodSignature> methodList, boolean add) {
    ...
    for (List<MethodSignature> methods :
        new TreeMap<String, List<MethodSignature>>(getMethodMap()).values())
