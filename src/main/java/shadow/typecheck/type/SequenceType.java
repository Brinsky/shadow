...
public class SequenceType extends Type implements Iterable<ModifiedType>, List<ModifiedType> {
  private List<ModifiedType> types;
  ...
  */
  ...
  public boolean canAccept(
      SequenceType inputTypes, SubstitutionKind substitutionType, List<ShadowException> errors) {
    ...
    for (int i = 0; i < types.size(); i++) {
      if (types.get(i) != null) {
        ...
        if (!BaseChecker.checkSubstitution(
            left, right, AssignmentKind.EQUAL, substitutionType, errors)) return false;
      }
    }
  ...
  public boolean canAccept(
      ModifiedType inputType, SubstitutionKind substitutionType, List<ShadowException> errors) {
    if (substitutionType.equals(SubstitutionKind.BINDING)) {
      ...
      for (ModifiedType modifiedType : types)
        if (!BaseChecker.checkSubstitution(
            modifiedType, inputType, AssignmentKind.EQUAL, substitutionType, errors)) return false;
  ...
  */
  ...
  @Override
  public SequenceType replace(List<ModifiedType> values, List<ModifiedType> replacements)
      ...
      if (type != null) {
        SimpleModifiedType dummy =
            new SimpleModifiedType(
                type.getType().replace(values, replacements), type.getModifiers());
        temp.add(dummy);
      } else temp.add(null);
    }
    ...
    for (int i = 0; i < types.size(); ++i) {
      ...
      if (type instanceof UninstantiatedType) {
        ...
        types.set(i, new SimpleModifiedType(type, types.get(i).getModifiers()));
          ...
          && ((ArrayType) type).recursivelyGetBaseType() instanceof UninstantiatedType) {
  ...
  public boolean matches(List<ModifiedType> inputTypes) {
    ...
    for (int i = 0; i < types.size(); i++) {
      if (inputTypes.get(i) == null || types.get(i) == null) return false;
      /*
      ...
      */
      ...
      if (!inputTypes.get(i).getType().equals(getType(i))) return false;
    }
