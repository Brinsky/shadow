...
public class ClassOrInterfacePage extends Page {
  ...
  public ClassOrInterfacePage(StandardTemplate master, Type type, Collection<Type> linkableTypes)
      throws DocumentationException {
    super(master);
    // It's necessary to put ClassType last since it's a base class for several others.
    if (type instanceof EnumType) typeKind = "Enum";
    else if (type instanceof ExceptionType) typeKind = "Exception";
    else if (type instanceof InterfaceType) typeKind = "Interface";
    else if (type instanceof SingletonType) typeKind = "Singleton";
    else if (type instanceof ClassType) typeKind = "Class";
    else
      throw new DocumentationException(
          "Unexpected type: " + type.toString(Type.PACKAGES | Type.TYPE_PARAMETERS));
    ...
    for (List<MethodSignature> overloadList : type.getMethodMap().values()) {
      for (MethodSignature method : overloadList) {
        if (!method.getModifiers().isPrivate()) {
          ...
          if (modifiers.hasModifier(Modifiers.GET) || modifiers.hasModifier(Modifiers.SET))
            properties.add(method);
          else if (method.getSymbol().equals("create")) constructors.add(method);
          else if (method.getSymbol().equals("destroy")) destructors.add(method);
            ...
            if (value == 0)
              value =
                  method1
                      .getParameterTypes()
                      .toString()
                      .compareTo(method2.getParameterTypes().toString());
            return value;
          }
        };
  ...
  public void write(Path root) throws IOException, ShadowException, DocumentationException {
    ...
    Path output =
        outputDirectory.resolve(type.toString(Type.NO_OPTIONS).replaceAll(":", "\\$") + EXTENSION);
  ...
  private void writeHeader(HtmlWriter out) throws ShadowException, DocumentationException {
    ...
    if (!packageName.isEmpty())
      writeLink(PackagePage.PAGE_NAME + EXTENSION, type.getPackage().getQualifiedName(), out);
    else writeLink(PackagePage.PAGE_NAME + EXTENSION, "default", out);
    out.closeLine();
    ...
    out.open("p");
    out.open("code");
    out.add(type.getModifiers().toString() + typeKind.toLowerCase() + " " + type.getTypeName());
  ...
  private void writeTypeParameters(Type parent, boolean link, HtmlWriter out)
    ...
    if (typeParams != null && typeParams.size() > 0) {
      ...
      for (ModifiedType modifiedParam : typeParams) {
        ...
        if (!classType.equals(Type.OBJECT)) bounds.add(classType);
        for (Type bound : param.getBounds()) if (bound instanceof InterfaceType) bounds.add(bound);
  ...
  private void writeInnerClassSection(HtmlWriter out)
    ...
    for (Type current : type.getInnerTypes().values()) {
      if (!current.getModifiers().isPrivate()) innerTypes.add(current);
    }

    if (!innerTypes.isEmpty()) out.fullLine("h4", "Visible inner classes");
    out.open("p");
    int i = 0;
  ...
  private void writeInheritanceSection(HtmlWriter out)
    ...
    if (type instanceof ClassType) extendType = ((ClassType) type).getExtendType();
  ...
  private void writeMethodSummaries(HtmlWriter out) throws ShadowException, DocumentationException {
    if (!constructors.isEmpty()) writeMethodTable("Create Summary", constructors, out);
    if (!destructors.isEmpty()) writeMethodTable("Destroy Summary", destructors, out);
    if (!methods.isEmpty()) writeMethodTable("Method Summary", methods, out);
    if (!properties.isEmpty()) writeMethodTable("Property Summary", properties, out);
  }

  private void writeMethodTable(String name, List<MethodSignature> methods, HtmlWriter out)
      throws ShadowException, DocumentationException {
    out.openTab("div", new Attribute("class", "block"));
    out.fullLine("h3", name);
    out.openTab("table", new Attribute("class", "summarytable"));

    writeTableRow(out, true, "Modifiers", "Return Types", "Method and Description");
    boolean shaded = false;
    for (MethodSignature method : methods) {
      ...
      out.close();
      if (method.hasDocumentation()) writeInlineTags(method.getDocumentation().getSummary(), out);
      out.closeLine();
      out.closeUntab();
      shaded = !shaded;
    }
  ...
  private void writeConstantDetails(HtmlWriter out) throws ShadowException, DocumentationException {
    if (!visibleConstants.isEmpty()) {
      out.openTab("div", new Attribute("class", "block"));
      out.fullLine("h3", "Constant Detail");

      for (Context constant : visibleConstants) writeConstantDetail(constant, out);
  ...
  private void writeConstantDetail(Context constant, HtmlWriter out)
    ...
    out.fullLine("h4", identifier, new Attribute("id", identifier));
  ...
  private void writeMethodDetails(HtmlWriter out) throws ShadowException, DocumentationException {
    if (!constructors.isEmpty()) writeMethodDetailSection("Create Detail", constructors, out);
    if (!destructors.isEmpty()) writeMethodDetailSection("Destroy Detail", destructors, out);
    if (!methods.isEmpty()) writeMethodDetailSection("Method Detail", methods, out);
    if (!properties.isEmpty()) writeMethodDetailSection("Property Detail", properties, out);
  }

  private void writeMethodDetailSection(String name, List<MethodSignature> methods, HtmlWriter out)
      throws ShadowException, DocumentationException {
    out.openTab("div", new Attribute("class", "block"));
    out.fullLine("h3", name);

    for (MethodSignature method : methods) writeMethodDetail(method, out);
  ...
  private void writeMethodDetail(MethodSignature method, HtmlWriter out)
    ...
    out.open("code");
    out.add(method.getModifiers().toString().trim() + " " + method.getSymbol());
    writeParameters(method, out);
    out.add(" => ");
    writeReturns(method, out);
    out.closeLine();
  ...
  private void writeIdentifier(Context node, boolean linkToDetail, HtmlWriter out)
      throws DocumentationException, ShadowException {
    String identifier = getIdentifier(node);

    if (linkToDetail) writeLink("#" + identifier, identifier, out);
    else out.add(identifier);
  }

  private void writeMethodName(MethodSignature method, boolean linkToDetail, HtmlWriter out)
      throws DocumentationException, ShadowException {
    if (linkToDetail) writeLink("#" + getUniqueID(method), method.getSymbol(), out);
  ...
  private void writeParamSection(List<List<String>> paramTags, HtmlWriter out)
      throws DocumentationException, ShadowException {
    if (paramTags.size() > 0) {
  ...
  private void writeThrowsSection(List<List<String>> throwsTags, HtmlWriter out)
      throws DocumentationException, ShadowException {
  ...
  private void writeReturnSection(List<List<String>> returnTags, HtmlWriter out)
      throws DocumentationException, ShadowException {
  ...
  private void writeCrossLink(Type to, int options, HtmlWriter out)
    ...
    if (to instanceof ArrayType) {
      ...
      out.add("[]");
        ...
        && linkableTypes.contains(to.getTypeWithoutTypeArguments())
        && (options & Type.TYPE_PARAMETERS) != 0) {
      Type current = to;
      ...
      while (!types.isEmpty()) {
        ...
        if (first) {
          first = false;
          writeCrossLink(
              current.getTypeWithoutTypeArguments(), options & ~Type.TYPE_PARAMETERS, out);
        } else {
      ...
      writeLink(
          getRelativePath(type, to).replaceAll(":", "\\$"),
          to.toString(options),
          new Attribute("title", to.toString(Type.PACKAGES | Type.TYPE_PARAMETERS)),
          out);
  ...
  private void writeCrossLink(Type to, String text, HtmlWriter out)
    ...
    if (linkableTypes.contains(to))
      ...
      writeLink(
          getRelativePath(type, to).replaceAll(":", "\\$"),
          text,
          new Attribute("title", to.toString(Type.PACKAGES)),
          out);
    else out.add(text);
  }
  ...
  /**
   * Creates a unique ID based on a method's signature, which can then be used as an HTML bookmark
   */
  private static String getUniqueID(MethodSignature method) {
    return method.getSymbol() + method.getParameterTypes().toString().replaceAll("\\s", "");
  }
