...
public class Main {
  ...
  public static void run(String[] args)
    ...
    if (isCompile) {
      ...
      if (!compileCSourceFiles(
          system.resolve(Paths.get("shadow", "c-source")).normalize(), cFiles, assembleCommand)) {
      ...
      Process compile =
          new ProcessBuilder(
              ...
              /* .redirectOutput(new File("a.s")) */ .redirectError(Redirect.INHERIT)
              .start();
      Process assemble =
          new ProcessBuilder(assembleCommand)
              ...
              .start();
      // Process assemble = new
      // ProcessBuilder(assembleCommand).redirectOutput(currentJob.getOutputFile().toFile())
  ...
  public static boolean shouldCompileCFile(Path currentFile, List<String> assembleCommand)
      throws IOException {
    Path assemblyPath =
        Paths.get(BaseChecker.stripExtension(currentFile.toString()) + ".s").normalize();
    assembleCommand.add(assemblyPath.toAbsolutePath().toString());

    if (currentJob.isForceRecompile()
        || !Files.exists(assemblyPath)
        || Files.getLastModifiedTime(assemblyPath).compareTo(Files.getLastModifiedTime(currentFile))
            < 0) {
   ...
   */
  private static void generateLLVM(List<Path> cFiles, List<String> linkCommand)
    ...
    try {
      for (Context node : typecheckerOutput.nodes) {
        ...
        if (currentJob.isCheckOnly()) {
          ...
          // For now, we don't generate modules for attributes - they exist only during
          // typechecking.
          ...
          if (node.isFromMetaFile()) {
            ...
            if (Files.exists(bitcodeFile)) linkCommand.add(canonicalize(bitcodeFile));
            else if (Files.exists(llvmFile)) linkCommand.add(optimizeLLVMFile(llvmFile));
            else throw new CompileException("File not found: " + bitcodeFile);
          } else {
          ...
          if (Files.exists(nativeBitcodeFile)) linkCommand.add(canonicalize(nativeBitcodeFile));
    ...
    try {
      optimize =
          new ProcessBuilder(
              ...
              .redirectError(Redirect.INHERIT)
              .start();
      if (optimize.waitFor() != 0) throw new CompileException("FAILED TO OPTIMIZE " + LLVMFile);
  ...
  private static String optimizeShadowFile(Path shadowPath, TACModule module)
